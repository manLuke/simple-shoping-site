...
export default {
  ...
  computed: {
    ...mapState(['isProductSelected']),
    quntity: {
      get () {
        return this.store.getQuantityById(this.product.id)
      },
      set (value) {
        this.useStore.setQuantityById({ id: this.product.id, quantity: value })
      }
    }
  },
  methods: {
    ...mapActions(useStore, ['addProduct', 'quantityPlus', 'quantityMinus'])
  }
}
</script>
