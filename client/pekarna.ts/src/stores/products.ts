...
export const useProductsStore = defineStore('products', {
  ...
  getters: {
    ...
    getSelectedProducts() {
      const obj: Product[] = [];
      this.selectedProducts.forEach(item => obj.push(this.products.find(p => p.id === item.id) as Product))
      return obj || [];
    },
    getProductById: (state: State) => (id: number) => state.products.find(product => product.id === id) || {},
    ...
    getPrice(): number {
      ...
      this.selectedProducts.forEach(item => {
        price += (item.quantity * ((this.products.find(product => product.id === item.id)?.price) || 0) || 0);
      })
      return Math.round((price + Number.EPSILON) * 100) / 100
    },
    getQuantity(): number {return this.selectedProducts.length},
    isProductSelected: (state: State) => (productId: number) => {return state.selectedProducts.some(product => product.id === productId)},
  },
  actions: {
